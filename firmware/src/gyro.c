...
void gyro_init(void)
{
    ...
    // Set up gyroscope.
    gyro_spi_write_register(GYRO_REG_CTRL2_G, (9<<4)|(3<<2)); // Enable gyro, 2000 dps, 3.33kHz Hz
    gyro_spi_write_register(GYRO_REG_CTRL4_C, (1<<1));        // Enable gyro low pass filter
    gyro_spi_write_register(GYRO_REG_CTRL6_C, 0);             // Low pass filter bandwidth = 328.5Hz
