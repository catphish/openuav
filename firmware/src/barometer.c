...
void baro_init(void) {
  ...
  gyro_spi_write_register(BARO_REG_CTRL_REG1, (4<<4)|(1<<3)|(1<<2));
}

uint32_t baro_read_pressure() {
  uint32_t out = gyro_spi_read_register(BARO_REG_PRESS_OUT_XL);
  out |= gyro_spi_read_register(BARO_REG_PRESS_OUT_L) << 8;
  out |= gyro_spi_read_register(BARO_REG_PRESS_OUT_H) << 16;
  return out;
}
